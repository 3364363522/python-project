...
from _02_model_likelihood import load_data, get_week_list, build_judge_total_long, build_events
from _02_model_likelihood import log_posterior, center_mu, vote_share
...
def run_mcmc(events, seasons, season_sizes,
    ...
    draws = {"gamma": [], "log_kappa": [], "mu_by_season": []}
    ...
    for it in range(n_iter):
        ...
        if (it + 1) % 1000 == 0:
            print(f"[w={w_percent}] iter {it+1}/{n_iter} lp={cur_lp:.2f} "
                  ...
                  f"steps=({step_mu:.3f},{step_gamma:.3f},{step_logk:.3f})", flush=True)

    accept_rates = {k: acc[k] / max(1, prop[k]) for k in acc}
    return draws, accept_rates


def weekly_elim_probs(ev, mu_s, gamma, kappa, w_percent):
    """
    返回 q：在 active_ids 上的淘汰概率分布（后验预测用）
    与似然一致：q ∝ exp(kappa * hazard)
    """
...
def export_all(df, judge_long, events, draws, accept_rates, outdir: Path, w_percent: float):
    ...
    logk_arr = np.array(draws["log_kappa"])
    kappa_arr = np.exp(logk_arr)

    global_rows = []
    global_rows.append({"param_name": "gamma", **summarize_array(gamma_arr)})
    global_rows.append({"param_name": "kappa", **summarize_array(kappa_arr)})
    ...
    for d in range(len(draws["gamma"])):
        gamma = draws["gamma"][d]
        kappa = float(np.exp(draws["log_kappa"][d]))
        mu_by_season = draws["mu_by_season"][d]
        for ev in events:
            if ev.skip_likelihood:
                continue
            q = weekly_elim_probs(ev, mu_by_season[ev.season], gamma, kappa, w_percent=w_percent)
    ...
    for ev in events:
        ...
        # pad to length 3 for output columns
        while len(top_ids) < 3:
            top_ids.append(None)
        ...
        check_rows.append({
            ...
            "notes": ""  # 你也可以把 no_elim / double_elim 等标进来
        })
...
def main(w_list=(0.5, 0.3, 0.7),
    ...
    seasons = sorted({e.season for e in events})
    season_sizes = {
        s: int(df.loc[df["season"] == s, "contestant_id"].max()) + 1
        for s in seasons
    }
    ...
    for w in w_list:
        ...
        try:
            ...
            (outdir / "run_error.txt").write_text(repr(e) + "\n", encoding="utf-8")
            raise


if __name__ == "__main__":
    # 仅运行 w=0.5
    main(w_list=(0.5,), n_iter=50000, burn=15000, thin=5, seed=0)
