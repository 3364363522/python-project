...
def _bottom2_judgesave_logprob_one_elim(
    ...
    for i in range(n):
        ...
        mask[i] = False
        hz2 = hz[mask]
        rem2_ids = [remaining_ids[k] for k in range(n) if mask[k]]
        ...
        for j, b2_id in enumerate(rem2_ids):
            ...
            # 注意：这里取 pair 的 J
            J_pair = np.array([J[i], hz2[j]*0.0], dtype=float)  # 占位，后面立刻用正确 J2
            # 修正第二个的 J：
            # rem2_ids[j] 对应原数组的索引 k2
            # 找到 b2 在 remaining_ids 里的位置：
            k2 = remaining_ids.index(b2_id)
