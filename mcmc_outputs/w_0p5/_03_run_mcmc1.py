...
from _02_model_likelihood import (
    ...
    hazard_percent, hazard_rank, log_softmax
)
...
def run_mcmc(
    ...
    w_percent=0.5, sigma_mu=4.0, seed=0
):
    rng = np.random.default_rng(seed)

    # ✅ 更宽的初始化（配合 sigma_mu 放宽）
    mu_by_season = {s: center_mu(rng.normal(0, 0.6, size=season_sizes[s])) for s in seasons}
    gamma = 0.0

    # ✅ 初始化：strict 更大、fuzzy 更小
    log_kappa_strict = float(np.log(15.0))
    ...
    for it in range(n_iter):
        ...
        if adapt and it < burn and (it + 1) % adapt_window == 0:
            for key in ["mu", "gamma", "logk_strict", "logk_fuzzy"]:
...
def _weekly_elim_probs_bottom2_save(ev, mu_s, gamma, kappa_fuzzy, w_percent):
    """
    与 _02_ 中 Bottom2 + Judge Save 的似然一致：
    返回在 ev.active_ids 上的淘汰概率 q
    """
    active_ids = list(ev.active_ids)
    n = len(active_ids)
    if n == 0:
        return np.array([])

    mu_vec = mu_s[np.array(active_ids, dtype=int)]
    p = vote_share(mu_vec, ev.zJ, gamma)

    if ev.rule == "percent":
        hz = hazard_percent(ev.j_percent, p, w=w_percent)
    else:
        hz = hazard_rank(ev.J, p)

    logp1 = log_softmax(kappa_fuzzy * hz)

    # q over all contestants
    log_q = np.full(n, -np.inf, dtype=float)
    ...
    for i in range(n):
        ...
        logp2 = log_softmax(kappa_fuzzy * hz2)

        for jj, j in enumerate(idx2):
            # pair = (i, j)
            hz_save = -np.array([ev.J[i], ev.J[j]], dtype=float)
            logp_save = log_softmax(kappa_fuzzy * hz_save)  # elim prob in pair

            # elim = i
            log_q[i] = np.logaddexp(log_q[i], logp1[i] + logp2[jj] + logp_save[0])
            # elim = j
            log_q[j] = np.logaddexp(log_q[j], logp1[i] + logp2[jj] + logp_save[1])

    # normalize
    m = np.max(log_q)
    q = np.exp(log_q - (m + np.log(np.sum(np.exp(log_q - m)) + 1e-300)))
    return q
...
def weekly_elim_probs(ev, mu_s, gamma, kappa_strict, kappa_fuzzy, w_percent):
    """
    后验预测用：与似然保持一致
    - S1-27：q ∝ exp(kappa_strict * hazard)
    - S28+ ：Bottom2 + Judge Save 的 q
    """
    if ev.season <= 27:
    ...
    return _weekly_elim_probs_bottom2_save(ev, mu_s, gamma, kappa_fuzzy, w_percent)
...
def export_all(df, judge_long, events, draws, accept_rates, outdir: Path, w_percent: float):
    ...
    # ---- vote share summary（仍按 events.active 输出；如你之前要“补齐周数”，可把那套导出逻辑替换到这里）
    ...
    # ---- weekly elimination check (posterior mean danger)
...
if __name__ == "__main__":
    main(w_list=(0.5,), n_iter=50000, burn=15000, thin=5, seed=0, sigma_mu=4.0)
