...
# ==========================================
# Figure 4: Boxplot Panel (Monochrome Style)
# ==========================================
def plot_metric_boxpanel(save_path='figure4_boxplots.png'):
    ...
    for ax, metric in zip(axes.flatten(), metrics):
        ...
        # 1. Draw Boxplot using palette (initially for patches)
        # We will override line colors manually to match perfectly.
        sns.boxplot(data=df_sims, x='mechanism', y=metric, order=display_order,
                    ax=ax, width=0.55, linewidth=1.5,
                    palette=DISPLAY_COLORS, # Base colors (display-name keys)
                    showfliers=True, # Show outliers
                    zorder=1,
                    flierprops=dict(marker='o', markersize=4))
        
        # 2. Iterate artists to apply "Transparent Fill + Solid Colored Stroke" style
        # Get mapping colors corresponding to the order on x-axis
        mech_colors = [COLORS[m] for m in mechanism_order]
        
        # A. Stylize Boxes (Patches)
        for i, patch in enumerate(ax.patches):
            if i < len(mech_colors):
                col = mech_colors[i]
                # Fill: Low saturation (Alpha 0.25)
                patch.set_facecolor(mcolors.to_rgba(col, 0.25))
                # Edge: Solid Color
                patch.set_edgecolor(col)
                patch.set_linewidth(1.5)
                
        # B. Stylize Lines (Whiskers, Caps, Medians, Fliers)
        # Seaborn/Matplotlib groups lines per box. 
        # Structure: 6 lines per box => [whisker, whisker, cap, cap, median, flier]
        # We iterate through them in chunks of 6 (or total_lines / num_boxes)
        
        num_boxes = len(mech_colors)
        lines_per_box = len(ax.lines) // num_boxes
        
        for i, col in enumerate(mech_colors):
            start_idx = i * lines_per_box
            end_idx = (i + 1) * lines_per_box
            
            # Apply color to all lines associated with this box
            for line in ax.lines[start_idx:end_idx]:
                line.set_color(col)
                line.set_linewidth(1.5) # Consistent stroke
                
                # Special handling for Fliers (Outliers) to make them open circles
                # Identify if it is a flier by checking marker
                if line.get_marker() == 'o':
                    line.set_markerfacecolor('none') # Open circle
                    line.set_markeredgecolor(col)
                    line.set_markeredgewidth(1.0)
                    line.set_alpha(0.8) # Slight alpha for fliers
